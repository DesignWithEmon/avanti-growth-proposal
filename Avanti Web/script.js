document.addEventListener('DOMContentLoaded', () => {
    ...
    function translatePage(lang) {
        ...
        window.AVANTI_OOS = [];
        fetch('/api/menu_stock')
            .then(res => res.json())
            .then(data => {
                if (data.success) window.AVANTI_OOS = data.data;
            })
    ...
    function renderMenu() {
        ...
        itemsToRender.forEach(item => {
            ...
            const dishDetailUrl = '/dish/' + item.name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
            ...
            if (imgPath) {
                imgHtml = `
                    <div class="menu-item-img-container" onclick="location.href='${dishDetailUrl}'">
                        ...
                        ${(window.AVANTI_OOS && window.AVANTI_OOS.includes(item.id.toString())) ? `<div style="position:absolute; top:10px; right:10px; background:var(--color-primary); color:#fff; padding:4px 8px; font-size:0.7rem; font-weight:600; border-radius:4px; z-index:2;">Out of Stock</div>` : ''}
            ...
            } else if (!hasPrice || (window.AVANTI_OOS && window.AVANTI_OOS.includes(item.id.toString()))) {
    ...
    if (goToCheckoutBtn) {
        goToCheckoutBtn.addEventListener('click', () => {
            ...
            if (checkoutFormContainer) {
                ...
                // Initialize Map if Delivery is selected
                if (deliveryType !== 'Takeaway' && typeof initCheckoutMap === 'function') {
    ...
    deliveryToggleBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (deliveryType === 'Takeaway') {
                ...
                if (typeof initCheckoutMap === 'function') {
                    setTimeout(initCheckoutMap, 200);
                }
            }
            ...
            if (isNameValid && isPhoneValid && isAddressValid) {
                ...
                .then(result => {
                    if (result.success) {
                        ...
                        // Automated Bot handles WhatsApp now
...
function initCheckoutMap() {
    ...
    const defaultLat = 24.7471;
    const defaultLng = 90.4203;
    ...
    if (!checkoutMap) {
        ...
        checkoutMarker.on('dragend', function(event){
...
function reverseGeocode(lat, lng) {
    fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lng}`)
        ...
        .then(data => {
            ...
            if(addressBox && data && data.display_name) {
                // Shorten the address for better UX
                const parts = data.display_name.split(', ');
                const shortAddress = parts.slice(0, 3).join(', ');
                addressBox.value = shortAddress;
                addressBox.classList.remove('has-error');
            }
        }).catch(err => console.log('Geocoding failed:', err));
}

// Attach locate button listener immediately (script is at end of body)
const locateBtn = document.getElementById('locate-me-btn');
if(locateBtn) {
    locateBtn.addEventListener('click', () => {
        const lbl = locateBtn.querySelector('.lbl-locate-me');
        if (lbl) lbl.innerText = 'Locating...';
        ...
        if ("geolocation" in navigator) {
            navigator.geolocation.getCurrentPosition((position) => {
                ...
                if (lbl) lbl.innerText = 'Locate Me (Live GPS)';
            }, (error) => {
